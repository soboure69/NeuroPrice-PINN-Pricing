...
def main() -> None:
    ...
    parser.add_argument("--checkpoint", type=str, default="artifacts/phase3_lookback_surrogate_offline/lookback_surrogate.pt")
    parser.add_argument("--out", type=str, default="artifacts/phase3_lookback_surrogate_offline/benchmark.json")
    ...
    if checkpoint.get("option_type") != "lookback_floating_call_surrogate" or checkpoint.get("output_scale") != "normalized_by_S_max":
        raise ValueError("Checkpoint is not compatible with lookback surrogate validation.")
    ...
    start = time.perf_counter()
    ref = lookback_floating_call_mc_np(S, tau, domain.r, domain.sigma, args.mc_paths, args.mc_steps, args.seed, args.mc_chunk_size)
