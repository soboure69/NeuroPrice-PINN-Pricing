...
def build_dataset(
    domain: LookbackSurrogateDomain,
    n_samples: int,
    n_paths: int,
    n_steps: int,
    chunk_size: int,
    ...
    target = lookback_floating_call_mc_np(S, tau, domain.r, domain.sigma, n_paths, n_steps, seed, chunk_size)
...
def evaluate(model: LookbackFloatingCallSurrogate, loader: DataLoader, device: torch.device, relative_floor: float) -> dict[str, float]:
    model.eval()
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", type=str, default="artifacts/phase3_lookback_surrogate_offline")
    ...
    domain = LookbackSurrogateDomain(r=args.r, sigma=args.sigma, T=args.T, S_max=args.S_max)
    ...
    model = LookbackFloatingCallSurrogate(hidden_dim=args.hidden_dim, hidden_layers=args.hidden_layers).to(device)
    ...
    checkpoint = {
        ...
        "option_type": "lookback_floating_call_surrogate",
    ...
    torch.save(checkpoint, out_dir / "lookback_surrogate.pt")
    (out_dir / "history.json").write_text(json.dumps(history, indent=2), encoding="utf-8")
    print(f"Saved model to {out_dir / 'lookback_surrogate.pt'}")
