...
from neuroprice.pinn.asian import AsianArithmeticPINN, AsianOptionDomain
...
def main() -> None:
    ...
    parser.add_argument("--checkpoint", type=str, default="artifacts/phase3_asian_arithmetic/asian_arithmetic_pinn.pt")
    parser.add_argument("--out", type=str, default="artifacts/phase3_asian_arithmetic/benchmark.json")
    ...
    if checkpoint.get("option_type") != "asian_arithmetic_call" or checkpoint.get("output_scale") != "normalized_by_S_max":
        raise ValueError("Checkpoint is not compatible with Asian arithmetic validation.")

    domain = AsianOptionDomain(**checkpoint["domain"])
    model = AsianArithmeticPINN(hidden_dim=int(checkpoint["hidden_dim"]), hidden_layers=int(checkpoint["hidden_layers"])).to(device)
    model.load_state_dict(checkpoint["model_state_dict"])
    model.eval()

    S = rng.uniform(1e-6, domain.S_max, size=(args.n_points, 1))
    A = rng.uniform(1e-6, domain.A_max, size=(args.n_points, 1))
    ...
    with torch.no_grad():
        pred_norm = model(S_norm, A_norm, tau_norm).detach().cpu().numpy()
    pinn_seconds = time.perf_counter() - start
    pred = domain.S_max * pred_norm

    start = time.perf_counter()
    ref = asian_arithmetic_call_mc_np(S, A, tau, domain.K, domain.r, domain.sigma, args.mc_paths, args.mc_steps, args.seed, args.mc_chunk_size)
    ...
    near_strike_mask = np.abs(A.reshape(-1) - domain.K) / domain.K <= 0.10
    result = {
