...
def load_surrogate(checkpoint_path: Path, device: torch.device) -> tuple[HestonCallSurrogate, HestonSurrogateDomain, dict[str, object]]:
    checkpoint = torch.load(checkpoint_path, map_location=device)
    if checkpoint.get("option_type") != "heston_call_surrogate":
        raise ValueError("Checkpoint is not compatible with heston_call_surrogate")
...
def monte_carlo_reference(rows: dict[str, np.ndarray], *, n_paths: int, n_steps: int, chunk_size: int, seed: int) -> tuple[np.ndarray, float]:
    n_points = rows["x"].shape[0]
    prices = np.zeros((n_points, 1), dtype=np.float32)
    start = time.perf_counter()
    for index in range(n_points):
        prices[index, 0] = heston_call_mc_np(
            S0=float(rows["spots"][index, 0]),
            K=float(rows["strikes"][index, 0]),
            r=float(rows["rates"][index, 0]),
            T=float(rows["maturities"][index, 0]),
            v0=float(rows["v0"][index, 0]),
            kappa=float(rows["kappa"][index, 0]),
            theta=float(rows["theta"][index, 0]),
            xi=float(rows["xi"][index, 0]),
            rho=float(rows["rho"][index, 0]),
            n_paths=n_paths,
            n_steps=n_steps,
            seed=seed + index,
...
def surrogate_predict(model: HestonCallSurrogate, x: np.ndarray, *, spot_max: float, device: torch.device) -> tuple[np.ndarray, float]:
...
def run_benchmark(
    *,
    ...
    ref, mc_seconds = monte_carlo_reference(rows, n_paths=mc_paths, n_steps=mc_steps, chunk_size=mc_chunk_size, seed=seed)
    metrics = summarize_errors(pred, ref, relative_floor)
    return {
        "instrument": "heston_call",
        "benchmark_version": "heston_surrogate_vs_mc_v1",
...
def main() -> None:
    ...
    parser.add_argument("--checkpoint", type=str, default="artifacts/phase6_heston_surrogate/heston_surrogate.pt")
    parser.add_argument("--dataset", type=str, default="artifacts/phase6_heston_surrogate_dataset/dataset.npz")
    parser.add_argument("--out", type=str, default="artifacts/phase6_heston_surrogate/benchmark.json")
    parser.add_argument("--n-points", type=int, default=500)
    parser.add_argument("--mc-paths", type=int, default=20000)
    parser.add_argument("--mc-steps", type=int, default=128)
