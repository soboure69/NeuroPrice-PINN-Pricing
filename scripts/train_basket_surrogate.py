...
def weighted_loss(pred: torch.Tensor, target: torch.Tensor, relative_floor: float) -> torch.Tensor:
    ...
    tradable_weight = 1.0 + 4.0 * (target > relative_floor).float()
    tradable_loss = torch.mean(tradable_weight * (pred - target) ** 2)
    return abs_loss + 0.05 * rel_loss + tradable_loss


def evaluate(model: BasketCallSurrogate, loader: DataLoader, device: torch.device, relative_floor: float) -> dict[str, float]:
    model.eval()
...
def train_surrogate(
    *,
    ...
    x, y, arrays = load_dataset(dataset_path)
    n_samples = x.shape[0]
    n_assets = arrays["spots"].shape[1]
    domain = BasketSurrogateDomain(n_assets=int(n_assets))
    ...
    model = BasketCallSurrogate(input_dim=domain.input_dim, hidden_dim=hidden_dim, hidden_layers=hidden_layers).to(device)
    ...
    if metadata_path is not None and metadata_path.exists():
        ...
        if source_metadata.get("dataset_version") != "basket_mc_dataset_v2":
            raise ValueError("Basket surrogate training requires dataset_version=basket_mc_dataset_v2")
    checkpoint = {
    ...
    torch.save(checkpoint, out_dir / "basket_surrogate.pt")
    (out_dir / "history.json").write_text(json.dumps(history, indent=2), encoding="utf-8")
    print(f"Saved model to {out_dir / 'basket_surrogate.pt'}")
    return history
...
def main() -> None:
    ...
    parser.add_argument("--metadata", type=str, default="artifacts/phase6_basket_surrogate_dataset/metadata.json")
    parser.add_argument("--out-dir", type=str, default="artifacts/phase6_basket_surrogate")
